...
{
	int i, j;
	vector3D_t vec[1000], vecbuff[1000], NormalVector;
	float normalzi;
...
void userdraw(void)
{
	static int tick = 50, dtick = 1, timeTick = 0;
	float theta = 0.5; bool stop = false;
	matrix3D_t mat; int count = 0;
	float stick = (tick % 300) / 200.;
	setColor(1., 1., 1.);
	matrix3D_t tilting = rotationXMTX(theta)*rotationYMTX(-theta);


	object3D_t kubus = { 8,{ { 200,-200,200 },{ 200,-200,-200 },{ -200,-200,-200 },{ -200,-200,200 },
	{ 200,200,200 },{ 200,200,-200 },{ -200,200,-200 },{ -200,200,200 } },
		6,{ { 4,{ 0,3,2,1 } },{ 4,{ 0,4,7,3 } },{ 4,{ 0,1,5,4 } },
		{ 4,{ 4,5,6,7 } },{ 4,{ 1,2,6,5 } },{ 4,{ 7,6,2,3 } } } };


	mat = tilting*translationMTX(tick, -tick * 3, 0)*rotationXMTX(tick / 15)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(-tick * 3, 100, tick * 3)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(tick * 20, 200, -tick)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(-tick * 3, -tick, 100)*rotationZMTX(tick / 15)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(tick * 5, 100, 100)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(-tick * 20, 200 + tick, 100)*rotationYMTX(tick / 15)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(tick * 5, 300, 100)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(tick * 5, 300, tick * 5)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	mat = tilting*translationMTX(0, 400 + tick, 50)*rotationYMTX(tick)*scalingMTX(0.25, 0.25, 0.25);
	draw3D(kubus, mat);
	...
	while (tick<0) {
		mat = tilting*translationMTX(0, 0, 0)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 100, 0)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 200, 0)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 0, 100)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 100, 100)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 200, 100)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 300, 100)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 300, 0)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		mat = tilting*translationMTX(0, 400, 50)*scalingMTX(0.25, 0.25, 0.25);
		draw3D(kubus, mat);
		count++;
		if (count == 6000)
		{
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Komposisi Transformasi Kubus");
	glClearColor(0.45, 0.6, 0.3, 0.8);
	gluOrtho2D(-420., 420., -440.0, 440.0);
